        ...
        InputProps={{
            ...
            <Stack sx={{width: "max-content"}}>
                <Stack sx={{position: "relative", display: openActions ? "inline-block" : "none"}}>
                    {
                    Actions.map((el) => {
                    return <Tooltip title={el.title} placement="right">
            ...
            <InputAdornment><IconButton onClick={() => { setOpenActions((prev) => !prev)}}><LinkSimple /></IconButton></InputAdornment>
            </Stack>, 
...
const Footer = () => {
  ...
  return (
    <Box p={2} sx={{width: "100%", backgroundColor: theme.palette.mode === "light" ? alpha(theme.palette.primary.main, 0.03)  : theme.palette.background.paper,  boxShadow:  "0px 0px 2px rgba(0,0,0,0.25)"}}>
